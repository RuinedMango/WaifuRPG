...
namespace std {
    template<>
    struct hash<Vertex> {
        size_t operator()(Vertex const &vertex) const { return ((hash<glm::vec3>()(vertex.pos) ^ (hash<glm::vec3>()(vertex.color) << 1)) >> 1) ^ (hash<glm::vec2>()(vertex.texCoord) << 1); }
    };
} // namespace std
...
private:
    ...
    void initVulkan() {
        ...
        createDescriptorSets(uniformBuffers, textureImageView, textureSampler, descriptorSets, descriptorSetLayout, descriptorPool);
        createDescriptorSets(uiUniformBuffers, uiTextureImageView, uiTextureSampler, uiDescriptorSets, uiDescriptorSetLayout, uiDescriptorPool);
    ...
    void createDescriptorSetLayout(VkDescriptorSetLayout& inDescriptorSetLayout) {
        ...
        VkDescriptorSetLayoutBinding uiSamplerLayoutBinding{};
        uiSamplerLayoutBinding.binding = 2;
        uiSamplerLayoutBinding.descriptorCount = 1;
        uiSamplerLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        uiSamplerLayoutBinding.pImmutableSamplers = nullptr;
        uiSamplerLayoutBinding.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

        std::array<VkDescriptorSetLayoutBinding, 3> bindings = {uboLayoutBinding, samplerLayoutBinding, uiSamplerLayoutBinding};
        VkDescriptorSetLayoutCreateInfo layoutInfo{};
        layoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
        layoutInfo.bindingCount = static_cast<uint32_t>(bindings.size());
        layoutInfo.pBindings = bindings.data();

        if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &inDescriptorSetLayout) != VK_SUCCESS) {
    ...
    void createGraphicsPipeline(std::string vertShader, std::string fragShader, VkPipelineLayout inPipelineLayout, VkPipeline inGraphicsPipeline) {
        ...
        if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &inGraphicsPipeline) != VK_SUCCESS) {
    ...
    void createTextureImage() {
        ...
        createImage(texWidth, texHeight, mipLevels, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, uiTextureImage, uiTextureImageMemory);

        transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);
        transitionImageLayout(uiTextureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);
    ...
    void createDescriptorPool(VkDescriptorPool &inDescriptorPool) {
        ...
        if (vkCreateDescriptorPool(device, &poolInfo, nullptr, &inDescriptorPool) != VK_SUCCESS) {
    ...
    void createDescriptorSets(std::vector<VkBuffer> inUniformBuffers, VkImageView inTextureImageView, VkSampler inTextureSampler,
        ...
        inDescriptorSets.resize(MAX_FRAMES_IN_FLIGHT);
        if (vkAllocateDescriptorSets(device, &allocInfo, inDescriptorSets.data()) != VK_SUCCESS) {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            imageInfo.imageView = inTextureImageView;
            imageInfo.sampler = inTextureSampler;

            std::array<VkWriteDescriptorSet, 2> descriptorWrites{};

            descriptorWrites[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            descriptorWrites[0].dstSet = inDescriptorSets[i];
            ...
            descriptorWrites[1].dstSet = inDescriptorSets[i];
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(uiIndices.size()), 1, 0, 0, 0);
